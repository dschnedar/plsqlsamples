...
---------------------------------------------------------------------------------
    ...
    matrix matrix_type;
    ----------------
    PROCEDURE p(p_str VARCHAR2) IS BEGIN dbms_output.put_line('>'||substr(p_str,1,254)); END p; ------ print
    ...
    IS
        ...
        digit_counts counter_type; ----- has 2 'methods' ( digit_counts$init , digit_counts$each_less_than_2 )
        
        PROCEDURE digit_counts$init -- zero out digit counter
        IS 
        BEGIN
            FOR i IN  1..9 LOOP     digit_counts(i) := 0;     END LOOP; 
        END digit_counts$init;
        
        FUNCTION digit_counts$each_less_than_2  RETURN BOOLEAN -- is each digit (1..9) used no more than once
        IS 
        BEGIN
            FOR i IN  1..9 LOOP   
            ...
            END LOOP;
            RETURN TRUE; 
        END digit_counts$each_less_than_2;
        -----------------------------------------------------------------
    BEGIN
        -----check rows--------
        FOR r IN 1..matrix.COUNT            LOOP
            ...
            END LOOP;
            RETURN digit_counts$each_less_than_2;
        END LOOP;
    ...
    PROCEDURE show   --------------- show puzzle structure for debugging ---------------
    IS 
        TmpVar NUMBER;
    ...
    BEGIN
        ...
        BEGIN
            IF rc > 81 THEN 
                RETURN TRUE;                    -- Puzzle solved, made it to square past/outside the puzzle 
            ELSIF matrix(r)(c) IS NOT NULL THEN
                RETURN solve(rc+1);             -- have value for square, go to next square 
            ELSE
                FOR i IN 1..9 LOOP              -- guess each digit for this square 
                    matrix(r)(c) := i;
                    IF valid THEN
                        RETURN solve(rc+1);     -- digit may work, go to next square
                    END IF;
                END LOOP;
                matrix(r)(c) := NULL;           -- no value works for this square
                RETURN FALSE;                   --      given existing values for previous squares 
            END IF;
        EXCEPTION 
