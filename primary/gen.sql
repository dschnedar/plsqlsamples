...
declare
    pk integer := null;
begin
    pk := ins_x_person( 4,'bob','bob@bobmail.com');
    dbms_output.put_line( 'bob[' || pk || ']' );   
    pk := ins_x_person( 5,'sue','sue@bobmail.com');
    dbms_output.put_line( 'sue[' || pk || ']' );   
    pk := ins_x_person( 6,'cathy','cathy@bobmail.com');
    dbms_output.put_line( 'cathy[' || pk || ']' );  
    -----------------------------------------------------------
    -----------------------------------------------------------
    pk := ins_x_person( null,'bob','bob@bobmail.com');
    dbms_output.put_line( 'bob[' || pk || ']' );   
    pk := ins_x_person( null,'sue','sue@bobmail.com');
    dbms_output.put_line( 'sue[' || pk || ']' );   
    pk := ins_x_person( null,'cathy','cathy@bobmail.com');
    dbms_output.put_line( 'cathy[' || pk || ']' );  
    -----------------------------------------------------------
    -----------------------------------------------------------
    pk := ins_x_person( p_id=>null,p_name=>'smith',p_email=>'smith@bobmail.com');
    dbms_output.put_line( 'smith[' || pk || ']' );   
    pk := ins_x_person(  p_name=> 'jones', p_email => 'jones@bobmail.com');
    dbms_output.put_line( 'jones[' || pk || ']' );   
    pk := ins_x_person( p_id=> 53 ,p_name=>'taylor',p_email=>'brown@bobmail.com');
    dbms_output.put_line( 'taylor[' || pk || ']' );  
    pk := ins_x_person( );
